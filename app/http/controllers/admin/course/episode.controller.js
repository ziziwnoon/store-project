...
const { getTime, deleteFileInPublic, deleteInvalidPropertiesInObject, copyObject } = require("../../../../utils/functions");
...
class EpisodeController extends Controller{
    async addEpisode(req, res, next){
        try {
            ...
            if(!createdEpisode.modifiedCount) throw createHttpError.InternalServerError("قسمت جدید ایجاد نشد")
    ...
    async editEpisode(req, res, next){
        try {
            ...
            const updatedEpisode = await CourseModel.updateOne({ "chapters.episodes._id" : episodeID} , {
                $set : {
                    "chapters.$.episodes" : newEpisode
                }
            })
            if(!updatedEpisode.modifiedCount) throw createHttpError.InternalServerError("قسمت موردنظر ویرایش نشد")
            return res.status(HttpStatus.CREATED).json({
                status : HttpStatus.CREATED,
                data:{
